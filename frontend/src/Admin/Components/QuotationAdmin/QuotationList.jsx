...
//                         className={`px-3 py-1 border rounded text-sm ${currentPage === 1 ? "bg-gray-200 text-gray-500" : "bg-white text-blue-500 hover:bg-blue-100"}`}
//                     >
//                         Prev
//                     </button>
...
//                                 className={`px-3 py-1 border rounded text-sm ${currentPage === page ? "bg-blue-500 text-white" : "bg-white text-blue-500 hover:bg-blue-100"}`}
//                             >
//                                 {page}
//                             </button>
//                         )
...
//                         className={`px-3 py-1 border rounded text-sm ${currentPage === totalPages ? "bg-gray-200 text-gray-500" : "bg-white text-blue-500 hover:bg-blue-100"}`}
//                     >
//                         Next
//                     </button>
//                 </div>
...
//             <div
//                 ref={hiddenRef}
//                 className="absolute -top-[10050px] -left-[9999px] w-[210mm] p-[20px] pt-[20px] pb-[20px] bg-white text-black font-sans overflow-visible"
...
//                             <div className="flex justify-between items-start text-[14px] leading-[1.8] text-[#01070dff]">
...
//                             </div>
//                             {/* Footer Message */}
//                             <div className="text-center mt-[40px] mb-[30px] text-[12px] text-gray-500">
...
import React, { useEffect, useState, useRef } from "react";
import axios from "axios";
import { FiEdit, FiTrash2, FiEye, FiDownload, FiPlus } from "react-icons/fi";
import html2canvas from "html2canvas";
import { jsPDF } from "jspdf";
import Swal from 'sweetalert2';
import QuotationForm from "./QuotationForm";
        ...
        <div className="bg-gray-50 min-h-screen p-4 md:p-6">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 md:mb-8">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Manage all your quotations here</p>
                </div>
                <button
                    onClick={() => { setSelectedQuotation(null); setIsModalOpen(true); }}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 overflow-y-auto">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl my-8 animate-fade-in">
            ...
            {isDetailModalOpen && (
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4 overflow-y-auto">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl my-8 animate-fade-in">
                        <div className="flex justify-between items-center p-5 border-b border-gray-100">
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Quotation No</th>
                            ...
                            {currentQuotations.length > 0 ? (
                                currentQuotations.map((q, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-800 font-medium">{q.customerName}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{q.serviceName}</td>
                                        <td className="px-6 py-4 text-sm text-gray-600">{q.contactNo}</td>
                                        <td className="px-6 py-4 text-right text-sm font-bold text-gray-900">₹{q.totalAmount.toLocaleString()}</td>
                                ...
                                <tr>
                                    <td colSpan="6" className="text-center py-12">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-gray-400 text-sm mt-1">Click "Create Quotation" to add your first quotation.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            <div className="md:hidden space-y-4">
                {currentQuotations.length > 0 ? (
                    currentQuotations.map((q) => (
                        <div key={q._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow duration-200">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                                    ...
                                    <span className="text-sm font-medium text-gray-800">{q.serviceName}</span>
                                </div>
                                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                                    <span className="text-xs text-gray-500">Contact No</span>
                                    <span className="text-sm font-medium text-gray-800">{q.contactNo}</span>
                                </div>
                                <div className="flex justify-between items-center pt-2">
                                    <span className="text-sm font-semibold text-gray-600">Total Amount</span>
                                    <span className="text-lg font-bold text-gray-900">₹{q.totalAmount.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    ))
                    ...
                    {totalPages > 5 && (
                        <>
                            ...
                            <button
                                ...
                                className={`w-10 h-10 rounded-lg text-sm font-medium transition-all duration-200 bg-white text-gray-700 hover:bg-gray-100 border border-gray-200`}
                            >
                                {totalPages}
                            </button>
                        </>
                            ...
                            <div className="flex justify-between items-start text-[14px] leading-[1.8] text-[#01070dff]">
                                ...
                                <div className="ml-4 flex-shrink-0 text-center mt-8">
                                    <img
                                        ...
                                        className="w-32 h-32 object-contain mx-auto"
                                    />
                                    <p className="-mt-1 text-[12px] text-black-600">Scan & Pay</p>
                                </div>
                            </div>
